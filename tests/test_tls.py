...
def test_the_bundled_root_is_the_certificate_it_claims_to_be() -> None:
    """Test that the bundled root hashes to its published fingerprint."""
...
def test_a_root_that_does_not_match_its_fingerprint_is_refused(
    ...
    """Test that a root failing its own fingerprint is refused rather than trusted."""
...
def test_the_context_adds_apples_root_to_the_platforms_own() -> None:
    """Test that Apple's root is added on top of the platform's trust store."""
    ...
    #
    # Compared as a subset rather than by counting. Whether the platform's own store is
    # loaded eagerly from a file or lazily from a hashed directory differs by platform and
    # by Python build -- `get_ca_certs()` returns one entry on a runner that loads lazily
    # and hundreds on one that does not -- so a count measures the machine, not this code.
    assert _common_names(ssl.create_default_context()) <= _common_names(context)
...
def test_verification_is_on_unless_it_is_turned_off() -> None:
    """Test that the default resolves to the verifying context."""
...
def test_disabling_verification_says_so(caplog: pytest.LogCaptureFixture) -> None:
    """Test that turning verification off is logged rather than silent."""
...
def test_the_opt_in_reaches_the_session_and_survives_a_round_trip() -> None:
    """Test that the opt-in reaches the session and is restored from JSON."""
...
def test_an_older_saved_provider_still_loads_and_verifies() -> None:
    """Test that a provider saved before this option existed still verifies."""
