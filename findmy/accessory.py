...
class FixedRollingKeyPairAccessoryMapping(TypedDict):
    """JSON mapping representing the state of a FixedRollingKeyPairAccessory instance."""

    type: Literal["custom_rolling_key_accessory"]
...
class FixedRollingKeyPairAccessory(
    ...
    """A findable Find My-accessory using a list of pre-generated keys."""

    def __init__(
        self, *, private_keys: list[bytes], name: str | None = None, identifier: str | None
    ) -> None:
        """
        Initialize a FixedRollingKeyPairAccessory. These values are usually generated by a script.

        :param private_keys: a list of pre-generated private keys associated with an accessory.
        """
    ...
    def private_keys(self) -> list[bytes]:
        """Private keys associated with this accessory."""
    ...
    @override
    def get_max_index(self, dt: datetime) -> int:
        return len(self._private_keys) - 1
    ...
    @override
    def keys_at(self, ind: int) -> set[KeyPair]:
        return {KeyPair(private_key=priv_key, name=self._name) for priv_key in self._private_keys}

    @override
    def to_json(
        self, path: str | Path | io.TextIOBase | None = None, /
    ) -> FixedRollingKeyPairAccessoryMapping:
        res: FixedRollingKeyPairAccessoryMapping = {
            "type": "custom_rolling_key_accessory",
            "private_keys": [private_key.hex() for private_key in self._private_keys],
            "name": self.name,
    ...
    def from_json(
        cls,
        val: str | Path | io.TextIOBase | io.BufferedIOBase | FixedRollingKeyPairAccessoryMapping,
        /,
    ) -> FixedRollingKeyPairAccessory:
        val = util.files.read_data_json(val)
        assert val["type"] == "custom_rolling_key_accessory"

        try:
            return cls(
                private_keys=[bytes.fromhex(private_key) for private_key in val["private_keys"]],
                name=val["name"],
    ...
    def __hash__(self) -> int:
        return hash(tuple([crypto.bytes_to_int(priv_key) for priv_key in self._private_keys]))

    @override
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, FixedRollingKeyPairAccessory):
            return False

        return set(self._private_keys) == set(other._private_keys)
