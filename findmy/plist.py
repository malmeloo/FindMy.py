...
from .errors import BadPlistError
from .util.crypto import decrypt_gcm
...
def decrypt_plist(encrypted: str | Path | bytes | IO[bytes], key: bytes) -> dict:
    ...
    if not isinstance(plist, list) or len(plist) < 3:
        raise BadPlistError(plist, "encrypted plist should be a list of 3 elements")

    nonce, tag, ciphertext = plist[0], plist[1], plist[2]
    decrypted_plist_bytes = decrypt_gcm(key=key, tag=tag, ciphertext=ciphertext, nonce=nonce)

    decrypted_plist = plistlib.loads(decrypted_plist_bytes)
    if not isinstance(decrypted_plist, dict):
        raise BadPlistError(decrypted_plist, "decrypted plist should be a dictionary")
...
def decrypt_all(out_dir: str | Path) -> list[FindMyAccessory]:
